...
// Check if a value is in the list (mutex assumed to be locked)
int mutex_member_locked(MutexLinkedList *list, int value)
{
    Node *current = list->head;
    while (current != NULL)
    {
...
void mutex_insert(MutexLinkedList *list, int value)
{
    ...
    if (!mutex_member_locked(list, value))
    { // Only insert if value is not already in the list
        Node *newNode = (Node *)malloc(sizeof(Node));
        if (newNode == NULL)
        {
            pthread_mutex_unlock(&list->mutex); // Handle malloc failure
            return;
        }
        newNode->data = value;
